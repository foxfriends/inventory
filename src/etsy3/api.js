...
class Etsy3 {
  ...
  async getInventory() {
    ...
    return Promise.all(
              ...
              map(
                applySpec({
                  name: pipe(
                    ...
                    map(
                      text` (${prop("property_name")}: ${o(join(", "), prop("values"))})`,
                    ),
                    join(""),
                  ),
                  sku: prop("sku"),
      ...
      chain(([listing, products]) =>
        products.map(evolve({ name: concat(listing.title) })),
      ),
    );
  }
  ...
  async setInventory(inventory) {
    ...
    return Promise.all(
      listings.map((listing) =>
        this.#getListing(listing).then(async (inventoryListing) => {
          ...
          if (equals(inventoryListing, newListing)) {
            return;
          }
          // Otherwise, Etsy's API is a real pain and we have to massage this data into another shape
          if (
            newListing.products.every(pathEq(["offerings", 0, "quantity"], 0))
          ) {
            ...
            disabledListing.price =
              disabledListing.price.amount / disabledListing.price.divisor;
            ...
            disabledListing.item_weight_unit =
              disabledListing.item_weight_unit ?? "g";
            disabledListing.item_dimensions_unit =
              disabledListing.item_dimensions_unit ?? "in";
            disabledListing.personalization_char_count_max =
              disabledListing.personalization_char_count_max ?? 0;
            try {
              await this.#client.put(
                `/application/shops/${this.#shop}/listings/${listing.listing_id}`,
                disabledListing,
                "application/x-www-form-urlencoded",
              );
            } catch (error) {
              log.warn(
                `Etsy updateListing(<${listing.listing_id} "${listing.title}">)`,
                error,
              );
              return listing.title;
            }
          } else if (
            inventoryListing.products.every(
              pathEq(["offerings", 0, "quantity"], 0),
            )
          ) {
            ...
            newListing.products = newListing.products.map(
              pipe(
                ...
                evolve({
                  ...
                  offerings: map(
                    pipe(
                      ...
                      evolve({
                        price: ({ amount, divisor }) => amount / divisor,
                      }),
                    ),
                  ),
                }),
            ...
            try {
              ...
              log.warn(
                `Etsy updateListingInventory(<${listing.listing_id} "${listing.title}">)`,
                error,
              );
              return listing.title;
            }
  ...
  async checkOrders() {
    ...
    return this.#client
      .getAll(`/application/shops/${this.#shop}/receipts`, {
        min_created: Math.floor(
          (
            (lastCheck && DateTime.fromISO(lastCheck)) ??
      ...
      .then(
        map(async (receipt) => {
          const { results } = await this.#client.get(
            `/application/shops/${this.#shop}/receipts/${receipt.receipt_id}/transactions`,
          );
          receipt.Transactions = await Promise.all(
            results.map(async (transaction) => {
              const listing = await this.#getListing(transaction);
              transaction.product_data = listing.products.find(
                whereEq({ product_id: transaction.product_id }),
              );
              return transaction;
            }),
          );
    ...
    if (!this.#ordersCron) {
      this.#ordersCron = new CronJob("0 0 * * * *", async () => {
        try {
          ...
          console.error(error, "Failed to check Etsy for orders");
        }
      });
